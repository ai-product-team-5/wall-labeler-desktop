...
from worker.io_utils import imread_unicode
from worker.models import CandidatePoint
...
def contour_candidates(binary: np.ndarray) -> list[CandidatePoint]:
    ...
    for contour in contours:
        ...
        for px, py in points:
            candidates.append(CandidatePoint(float(px), float(py), score, "contour"))
...
def segment_endpoint_candidates(
    ...
    for index in range(segment_count):
        ...
        for j in range(index + 1, segment_count):
            ...
            if (
                px < -tolerance
                or py < -tolerance
                or px > width + tolerance
                or py > height + tolerance
            ):
                continue
...
def cluster_points(
    ...
    for point in sorted_candidates:
        ...
        best_cluster["x"] = (
            best_cluster["x"] * best_cluster["weight"] + point.x * point.score
        ) / weight
        best_cluster["y"] = (
            best_cluster["y"] * best_cluster["weight"] + point.y * point.score
        ) / weight
